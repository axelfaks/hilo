    ...
    with sesion() as s:
        ...
        for c in CLIENTES:
            ...
            for canal, valor in c["identidades"]:
                # MAIL_CLIENTE_DEMO le pone una dirección REAL al primer cliente,
                # para poder mandarle un mail de verdad y ver el ida y vuelta
                # completo sin tener que fusionar nada. Sobrevive a cada reset,
                # que es lo que importa cuando se ensaya la demo tres veces.
                if canal == "mail" and c["nombre"] == "Panadería La Espiga":
                    real = (os.environ.get("MAIL_CLIENTE_DEMO") or "").strip()
                    if real:
                        valor = real
